...
public class BottomTabStrip extends LinearLayout {
    private static final String TAG = BottomTabStrip.class.getName();
    ...
    private TabStripListener mTabStripListener;
    ...
    public BottomTabStrip(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public BottomTabStrip(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if ((mMode & TabLayoutMode.HIDE_TEXT) > 0) {
            ...
            if (width_dp > max_w) {
                ...
                mDefaultWidth = (int) Utils.dp2px(mContext, HIDE_TEXT_MAX_WIDTH);
            } else {
                ...
                mDefaultWidth = (int) Utils.dp2px(mContext, default_width);
                mSelectedWidth = (int) Utils.dp2px(mContext, selected_width);
            }

            setTabItemWidth(mDefaultWidth);
            ...
            if (w > DEFAULT_MAX_WIDTH) {
                ...
                setPadding(leftPadding, 0, 0, 0);
                mSelectedWidth = mDefaultWidth = (int) Utils.dp2px(mContext, DEFAULT_MAX_WIDTH);
            } else {
                mSelectedWidth = mDefaultWidth = (int) Utils.dp2px(mContext, w);
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if ((mMode & TabLayoutMode.HIDE_TEXT) > 0 && mIndex != mOldIndex) {
            ...
            if (old_width != mDefaultWidth || new_width != mSelectedWidth) {
                int n = mSelectedWidth - mDefaultWidth;
                old_width -= n < 10 ? 1 : n / 10;
                new_width += n < 10 ? 1 : n / 10;

                old_width = old_width < mDefaultWidth ? mDefaultWidth : old_width;
    ...
    protected void invalidateView() {
        if (Looper.getMainLooper() == Looper.myLooper()) {
            ...
            BottomTabStrip.this.postInvalidate();
        }
    }
     ...
     * @param index
     */
    public void setSetectManually(int index) {
        if (mIndex == index) {
            return;
        }
        //不应该存在的索引错误
        ...
        if (mDefaultWidth == 0) {
            mIndex = index;
            mTabStripListener.onNotMeasure(mTabItems.get(mIndex).getSelectedColor());
            return;
        }
        ...
        //回调，主要用于背景颜色切换的模式下
        float x = mOldIndex > mIndex ? (mIndex + 0.5f) * mDefaultWidth : (mIndex - 0.5f) * mDefaultWidth + mSelectedWidth;
        float y = Utils.dp2px(mContext, 28);
        mTabStripListener.onSelect(x, y);
    ...
    public TabStripBuild builder(TabStripListener onFinishBuild) {
        mTabItems.clear();
        BottomTabStrip.this.removeAllViews();
        this.mTabStripListener = onFinishBuild;
        return new builder();
    }
    ...
    class builder implements TabStripBuild {
        ...
        public Controller build() {
            for (int i = 0; i < mTabItems.size(); i++) {
                ...
                BottomTabStrip.this.addView(item);
            ...
            return mTabStripListener.onFinishBuild();
        }
